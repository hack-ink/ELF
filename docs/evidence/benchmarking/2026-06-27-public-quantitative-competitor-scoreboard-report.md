...
| OpenViking | `blocked` | `live_baseline` | `false` | `false` / `false` | Local embedding setup passes, but retrieval/context-trajectory rows remain non-pass. | Live-baseline and OpenViking report artifacts in snapshot `source_provenance[]`. |
...
| claude-mem | `blocked` | `live_baseline` | `false` | `false` / `false` | Durable/progressive-disclosure strengths exist; retrieval/capture/operator blockers remain. | claude-mem live-baseline artifacts in snapshot `source_provenance[]`. |
...
| graphify | `blocked` | `live_real_world` | `false` | `true` / `false` | Docker graph-report generation reaches runtime, but current scored row is wrong-result/blocked and lacks digest evidence. | graphify smoke artifacts in snapshot `source_provenance[]`. |
...
| qmd | `blocked` | `live_real_world` | `false` | `true` / `false` | CLI retrieval/replay and targeted live-pass strengths exist, but current full live-real-world rows include wrong-result/not-encoded/blocker states and no digest evidence. | qmd live-adapter and live-baseline artifacts in snapshot `source_provenance[]`. |
