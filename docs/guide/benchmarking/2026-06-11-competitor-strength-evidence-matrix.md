...
- ELF and qmd have full-suite `live_real_world` sweeps, but neither has a full-suite
  live pass. The fresh ELF sweep produced 38 jobs with 18 pass, 5 wrong_result,
  0 incomplete, 2 blocked, and 13 not_encoded; the fresh qmd sweep produced 17 pass,
  6 wrong_result, 0 incomplete, 2 blocked, and 13 not_encoded.
...
`research_gate`. Overall adapter-status counts: 3 `pass`, 4 `wrong_result`, 1
...
| Temporal/current-vs-historical memory | Fixture memory_evolution passes; live memory_evolution is `wrong_result`. | Graphiti/Zep, mem0/OpenMemory. | Graphiti/Zep is `research_gate` `blocked`; mem0/OpenMemory basic local lifecycle now passes, but preference/entity history, deletion audit, UI/export, and graph-memory scenarios are `not_encoded`. | Fix ELF/qmd live memory_evolution evidence links, encode mem0/OpenMemory history/UI jobs, and run XY-888. |
