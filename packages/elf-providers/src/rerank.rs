...
mod tests {
	...
	fn aligns_scores_by_index() {
		...
		let scores = rerank::parse_rerank_response(json, 2)
	...
	fn local_rerank_scores_match_token_overlap_fraction() {
		let scores =
			rerank::local_rerank("alpha beta", &[String::from("alpha"), String::from("gamma")]);
	...
	fn local_noisy_model_is_detected_and_nonnegative() {
		assert_eq!(rerank::parse_local_noisy_model("local-token-overlap"), None);
		assert_eq!(rerank::parse_local_noisy_model("local-token-overlap-noisy@0.02"), Some(0.02));
		assert_eq!(rerank::parse_local_noisy_model("local-token-overlap-noisy@-1"), Some(0.0));
	}
	...
	fn local_rerank_noisy_varies_across_calls() {
		...
		let first =
			rerank::local_rerank_dispatch("local-token-overlap-noisy@0.1", "alpha beta", &docs);
