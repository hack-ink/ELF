...
use elf_service::{
	AddNoteInput, AddNoteRequest, BoxFuture, DocsDeleteRequest, DocsExcerptsGetRequest,
	DocsGetRequest, DocsPutRequest, DocsPutResponse, DocsSearchL0Request, ElfService,
	EmbeddingProvider, Error, NoteOp, PayloadLevel, Providers, Result, SearchRequest,
	TextQuoteSelector, docs::DocRetrievalTrajectory,
};
...
async fn docs_delete_marks_doc_deleted_and_removes_doc_vectors() {
	...
	assert!(
		fetch_first_doc_chunk_point(&service, put.doc_id).await.is_some(),
	...
	assert!(
		fetch_first_doc_chunk_point(&service, put.doc_id).await.is_none(),
