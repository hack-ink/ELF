...
async fn insert_note_with_importance<'e, E>(
	executor: E,
	note_id: Uuid,
	note_text: &str,
...
async fn seed_relation_context_fixture(
	...
	.await;
	upsert_point(service, chunk_id, note_id, 0, 0, chunk_text.len() as i32, chunk_text).await;
...
async fn search_filter_affects_candidate_set_and_records_filter_impact() {
	let provider = build_providers(StubRerank);
	...
	let mut context = match setup_context(
