...
impl ElfService {
	pub async fn add_note(&self, req: AddNoteRequest) -> Result<AddNoteResponse> {
		...
		for note in notes {
			if let Some(rejected) = reject_invalid_structured(note.structured.as_ref(), &note) {
				results.push(rejected);

				continue;
			}
			if let Some(rejected) = self.reject_by_writegate(request_ids.scope, &note) {
				results.push(rejected);

				continue;
			}

			results.push(self.process_note(&request_ids, note, now, embed_version.as_str()).await?);
		}

		Ok(AddNoteResponse { results })
	}

	fn reject_by_writegate(&self, scope: &str, note: &AddNoteInput) -> Option<AddNoteResult> {
		...
		writegate::writegate(&gate_input, &self.cfg).err().map(|code| AddNoteResult {
			note_id: None,
			op: NoteOp::Rejected,
	...
	async fn process_note(
		&self,
		request_ids: &AddNoteRequestIds<'_>,
		note: AddNoteInput,
		now: OffsetDateTime,
		embed_version: &str,
		...
		let decision = crate::resolve_update(
			...
			ResolveUpdateArgs {
				...
				tenant_id: request_ids.tenant_id,
				project_id: request_ids.project_id,
				agent_id: request_ids.agent_id,
				scope: request_ids.scope,
		...
		match decision {
			UpdateDecision::Add { note_id } =>
				self.apply_add_update(&mut tx, request_ids, &note, note_id, now, embed_version)
					.await,
			UpdateDecision::Update { note_id } =>
				self.apply_existing_update(&mut tx, &note, note_id, now).await,
			UpdateDecision::None { note_id } =>
				self.apply_none_update(&mut tx, &note, note_id, now, embed_version).await,
		}
	}

	async fn apply_add_update(
		&self,
		...
		let expires_at = ttl::compute_expires_at(note.ttl_days, &note.r#type, &self.cfg, now);
		let memory_note = MemoryNote {
			note_id,
			tenant_id: request_ids.tenant_id.to_string(),
			project_id: request_ids.project_id.to_string(),
			agent_id: request_ids.agent_id.to_string(),
			scope: request_ids.scope.to_string(),
...
VALUES (
	...
	$18
		...
		{
			upsert_structured_fields_tx(tx, memory_note.note_id, structured, now).await?;
		}
	...
	async fn apply_existing_update(
		&self,
		tx: &mut sqlx::Transaction<'_, sqlx::Postgres>,
		note: &AddNoteInput,
		note_id: Uuid,
		...
		let expires_at = match requested_ttl {
			Some(ttl) => ttl::compute_expires_at(Some(ttl), &note.r#type, &self.cfg, now),
...
WHERE note_id = $7",
		...
		if let Some(structured) = note.structured.as_ref()
			&& !structured.is_effectively_empty()
		{
			upsert_structured_fields_tx(tx, existing.note_id, structured, now).await?;
		}
	...
	async fn apply_none_update(
		...
		if let Some(structured) = note.structured.as_ref()
			&& !structured.is_effectively_empty()
		{
			upsert_structured_fields_tx(tx, note_id, structured, now).await?;
...
fn validate_note_language(notes: &[AddNoteInput]) -> Result<()> {
...
fn reject_invalid_structured(
	structured: Option<&StructuredFields>,
	note: &AddNoteInput,
) -> Option<AddNoteResult> {
	let structured = structured?;

	if let Err(err) = validate_structured_fields(structured, &note.text, &note.source_ref, None) {
