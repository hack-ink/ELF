...
pub(crate) fn parse_pg_vector(text: &str) -> Result<Vec<f32>, ServiceError> {
    ...
    let without_brackets = trimmed
        ...
        .ok_or_else(|| ServiceError::InvalidRequest {
            message: "Vector text is not bracketed.".to_string(),
        })?;
...
pub(crate) async fn resolve_update(
    ...
    if let Some(key) = key.filter(|value| !value.trim().is_empty()) {
        if let Some(note_id) = sqlx::query_scalar::<_, uuid::Uuid>(
    ...
    let embeddings = providers
        .embedding
        .embed(&cfg.providers.embedding, &[text.to_string()])
        .await?;
