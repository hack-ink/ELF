...
def mkdirs() -> None:
    ...
    for path in (OUT, MANIFEST_OUT, SUMMARY_OUT, REPORT_DIR / "generated-corpus.csv"):
        if path.exists():
            path.unlink()
...
def write_fixture(corpus: list[CorpusItem], status: StatusState, mapped_ids: list[str]) -> Path:
    ...
    fixture: dict[str, Any] = {
        ...
        "allowed_uncertainty": {
            "phrases": ["tiny generated corpus", "derived graph/report adapter"],
            "fallback": "Report typed failure when graphify output cannot be mapped to evidence ids.",
        },
        "operator_debug": None,
        "encoding": {},
