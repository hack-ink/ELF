...
use crate::{
	routes::{self, ADMIN_VIEWER_PATH, MAX_DOC_REQUEST_BYTES, MAX_REQUEST_BYTES},
	state::AppState,
};
...
pub fn router(state: AppState) -> Router {
	...
	Router::new()
		...
		.merge(
			docs_api_router().with_state(state).layer(DefaultBodyLimit::max(MAX_DOC_REQUEST_BYTES)),
		)
...
fn docs_api_router() -> Router<AppState> {
	Router::new()
