...
pub(crate) fn assert_openapi_method(spec: &serde_json::Value, path: &str, method: &str) {
	let operation = spec
		.get("paths")
...
pub(crate) async fn list_org_shared_notes_as_reader(app: &Router) -> serde_json::Value {
	let response = app
		.clone()
		.oneshot(
			Request::builder()
...
pub(crate) async fn contract_json() -> serde_json::Value {
