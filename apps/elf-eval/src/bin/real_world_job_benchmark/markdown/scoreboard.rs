use crate::markdown::{
	self, RealWorldReport, SCOREBOARD_EVIDENCE_CLASSES, ScoreboardReport, ScoreboardRow,
};
...
pub(super) fn render_markdown_scoreboard(out: &mut String, report: &RealWorldReport) {
	...
	out.push_str(&format!(
		"- Schema: `{}`\n",
		markdown::md_inline(report.scoreboard.schema.as_str())
	));
	out.push_str(&format!(
		"- Result states: `{}`\n",
		markdown::md_inline(report.scoreboard.result_states.join(", ").as_str())
	));
	out.push_str(&format!(
		"- Evidence classes: `{}`\n",
		markdown::md_inline(report.scoreboard.evidence_classes.join(", ").as_str())
	));
	out.push_str(&format!(
		"- Metric basis: `{}` at k=`{}`\n",
		markdown::md_inline(report.scoreboard.metric_basis.as_str()),
		report.scoreboard.retrieval_k
	));
	out.push_str(&format!(
		"- Summary claim: `{}`\n",
		markdown::md_inline(report.scoreboard.summary_claim.as_str())
	));
	out.push_str(&format!(
		"- Job summary claim: `{}`\n",
		markdown::md_inline(report.scoreboard.job_summary_claim.as_str())
	));
	out.push_str(&format!(
	...
	out.push_str(&format!(
		...
		markdown::md_inline(
			scoreboard_state_list(&report.scoreboard.typed_non_pass_states_present).as_str()
		)
	));
	out.push_str(&format!(
		"- Evidence class counts: `{}`\n",
		markdown::md_inline(scoreboard_evidence_class_count_display(&report.scoreboard).as_str())
	));
	out.push_str(&format!(
	...
	out.push_str(&format!(
		"- Claim boundary: {}\n\n",
		markdown::md_cell(report.scoreboard.claim_boundary.as_str())
	));
	...
	for row in &report.scoreboard.rows {
		out.push_str(&format!(
			"| {} | `{}` | `{}` | `{}` | {} | {} | {} | {} | {} | {} | {} | {} | {} | {} |\n",
			markdown::md_cell(row.product_name.as_str()),
			...
			markdown::md_cell(scoreboard_list_cell(&row.next_evidence).as_str())
		));
	}
...
fn scoreboard_optional_f64_plain(value: Option<f64>) -> String {
	value.map_or_else(|| "n/a".to_string(), |value| markdown::round3(value).to_string())
}
...
fn scoreboard_latency_cell(row: &ScoreboardRow) -> String {
	row.metrics.operations.mean_latency_ms.map_or_else(
		|| "`n/a`".to_string(),
		|latency| format!("`{} ms`", markdown::round3(latency)),
	)
}
