use crate::{
	HashMap, IngestedCorpus, KnowledgePageLintResponse, KnowledgePageResponse, LoadedJob, Result,
	Uuid, knowledge_adapter::traps, serde_json,
};

pub(crate) fn knowledge_page_artifact(
	loaded: &LoadedJob,
	...
	lint.findings
		...
		.map(|finding| {
			...
			let trap_id = evidence_ids
				.first()
				.and_then(|evidence_id| traps::trap_id_for_evidence(loaded, evidence_id));
