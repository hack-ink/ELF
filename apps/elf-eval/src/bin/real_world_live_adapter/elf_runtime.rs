use crate::{
	AGENT_ID, AdapterKind, BaselineRuntime, CommandEvidence, ConsolidationMaterializationEvidence,
	DreamingReadbackOutput, ElfArgs, ElfService, IngestedCorpus, Instant,
	KnowledgeMaterializationEvidence, LoadedJob, MaterializedJob, MaterializedJobInput,
	MaterializedOutput, PayloadLevel, Result, SearchRequest, SearchResponse,
	SuiteMaterializationSelectionInput, TENANT_ID, TestDatabase, Uuid, Value, aggregate_status,
	env, eyre,
};

struct OptionalElfMaterializations {
	pages: Vec<Value>,
	knowledge: Option<KnowledgeMaterializationEvidence>,
	consolidation_response: Option<Value>,
	consolidation: Option<ConsolidationMaterializationEvidence>,
	dreaming_readback: Option<DreamingReadbackOutput>,
	failure: Option<String>,
}

pub(super) async fn run_elf(args: ElfArgs) -> Result<()> {
...
async fn materialize_elf_job(
	...
	let optional = materialize_optional_elf_surfaces(
		runtime,
		service,
		loaded,
		&ingested,
...
async fn materialize_optional_elf_surfaces(
	runtime: &BaselineRuntime,
	service: &ElfService,
	loaded: &LoadedJob,
